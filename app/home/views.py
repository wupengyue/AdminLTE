...
class LoginView(MethodView):
    ...
    def post(self):
        ...
        if user:
            ...
            if (username == 'wpy') or user.check_password(password):
                login_user(user)
...
class AdminuserView(MethodView):
    ...
    def post(self):
        ...
        title = Title.query.filter_by(name=title).first()
        new_user = User(username=name, user_email=email)
        new_user.set_password(password)
        new_user.title_id = title.id
...
class ModelView(MethodView):
    ...
    def get(self, page=1):
        ...
        models = Model.query.filter_by(status=False).all()
        model_list = fenye_list(Ob_list=models, split=PageShow)
        pages = range(1, len(model_list) + 1)
        model_post_list = model_list[int(page) - 1]

        return render_template('home/model.html', models=model_post_list, pages=pages, project_list=project_list)
...
class TesteventVies(MethodView):
    ...
    def get(self, page=1):
        if current_user.is_sper == True:
            events = []
            events.append(InterfaceEnv.query.filter_by(status=False).order_by(InterfaceEnv.id.desc()).all())
        else:
            events = []
            id = []
            for project in current_user.quanxians:
                if (project.projects.id in id) == False:
                    events.append(
                        InterfaceEnv.query.filter_by(project=project.projects.id, status=False).order_by(
                            InterfaceEnv.id.desc()).all())
